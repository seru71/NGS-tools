#!/usr/bin/perl -w
#
# Provided by pruzanov on Biostars [1]
# Downloaded and modified by Paweł Sztromwasser
# Dec 2014
#
#
...
sub blast2sam {
  ...
  @cmaux = (0, 0, 0, '');
  while (<>) {
    if ($cmaux[1]>0 && (/^Query=/ || /Score =.*bits.*Expect/ || /^> \S+/)) {
#
#   replaced the check below with the one above as perfectly matching hits in a first record were skipped
#
